...
(function(){
  ...
  function pdfFromSession(s){
    ...
    const section=t=>{
      // Keep the section title and at least the table header on the same page.
      ensure(80);
      txt(L+3,y-12,t,7.2,true);rect(L,y-19,W,19);y-=19;
    };
    const table=(headers,rows,widths)=>{
      ...
      const drawRow=(cells,isHeader)=>{
        ...
        if(y-h<BOTTOM){pages.push({content:ops});ops='';pageHeader(reportTitle,reportMeta,true);return drawRow(cells,isHeader)}
        let x=L;
        cells.forEach((v,i)=>{
      ...
      drawRow(headers,true);rows.forEach(r=>drawRow(r,false));y-=5;
    };
    ...
    if(s.selectedReports?.includes('altimetro')){
    ...
    if(s.selectedReports?.includes('encoder')){
    ...
    if(s.selectedReports?.includes('transponder')){
    ...
    if(s.selectedReports?.includes('airdata')){
    ...
    if(s.observacoesAjustes&&String(s.observacoesAjustes).trim()){
      if(!pages.length){pageHeader('OBSERVAÇÕES / AJUSTES',rmFor(s.selectedReports?.[0]||'altimetro'));}
