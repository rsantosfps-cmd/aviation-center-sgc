...
function showView(v){
 ...
 const titles={dashboard:["Dashboard","Gerenciamento de calibração"],session:["Nova calibração","Preenchimento dos laudos"],sessions:["Calibrações","Calibrações salvas"],instruments:["Instrumentos","Instrumentos registrados nas calibrações"],clients:["Clientes","Clientes e histórico das aeronaves"],aircraft:["Aeronaves","Histórico de serviços por prefixo"],history:["Histórico","Histórico completo de serviços"],trash:["Lixeira","Calibrações apagadas nos últimos 7 dias"],backup:["Backup","Dados locais do Aviation Center Calibrações"],settings:["Configurações","Configurações do sistema"]};
...
function openPdfInViewer(blob,filename){
 ...
 // Fallback determinístico para Safari/PWA: abre o PDF na própria janela.
 // O visualizador nativo do iPad permite Imprimir e Salvar em Arquivos.
 try{window.location.href=url;}catch(e){alert("Não foi possível abrir o PDF neste navegador.");}
 // A URL precisa continuar viva enquanto o visualizador estiver aberto.
 setTimeout(()=>URL.revokeObjectURL(url),300000);
}

async function printReport(){
 const result=makeCurrentPDF();
 if(!result)return;
 const {blob,filename}=result;
 const file=new File([blob],filename,{type:"application/pdf"});
 // No iPad/iPhone, a folha nativa de compartilhamento é o caminho mais estável
 // para chegar ao comando Imprimir quando o app está instalado como PWA.
 try{
   if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
     await navigator.share({files:[file],title:filename,text:"PDF da calibração. Escolha Imprimir ou Salvar em Arquivos."});
     return;
   }
...
async function downloadReport(){
 ...
 try{
   if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
     await navigator.share({files:[file],title:filename,text:"Salve este PDF em Arquivos."});
     return;
   }
 }catch(e){if(e?.name==="AbortError")return;console.warn(e)}
 // Fallback de download para navegadores que suportam download de Blob.
 const url=URL.createObjectURL(blob);
 const a=document.createElement("a");a.href=url;a.download=filename;a.target="_blank";a.rel="noopener";a.textContent="Abrir PDF";a.style.position="fixed";a.style.left="-9999px";document.body.appendChild(a);a.click();a.remove();
