...
function blankSession(){return {id:uid(),status:"Nova",createdAt:new Date().toISOString(),selectedReports:["altimetro","integracao","encoder","transponder","airdata"],meta:{os:"",data:new Date().toISOString().slice(0,10),cliente:"",validade:"2 anos",fabricante:"",pn:"",sn:"",prefixo:"",tecnico:""},reports:{altimetro:{friction:ALT_FRICTION.map(()=>({sem:"",com:""})),scale:ALT_SCALE.map(()=>({lido:""})),leak:{pressao:"14,942",lido:""},baro:BARO.map(x=>({valor:x[0],diferenca:x[1],lido:""})),hyst:[{lidoDec:"",lidoCresc:"",dif:"",altitude:10000},{lidoDec:"",lidoCresc:"",dif:"",altitude:8000}]},integracao:INT_ALT.map((a,i)=>({teste:i+1,altitude:a,altimetro:"",transponder:""})),encoder:ENC_NOM.map((n,i)=>({teste:i+1,altitude:INT_ALT[i],nominal:n,atual:""})),transponder:{radio:[
...
function fillSession(s){currentId=s.id; set("os",s.meta.os);set("data",s.meta.data);set("cliente",s.meta.cliente);set("validade",s.meta.validade);set("fabricante",s.meta.fabricante);set("pn",s.meta.pn);set("sn",s.meta.sn);set("prefixo",s.meta.prefixo);set("tecnico",s.meta.tecnico);set("observacoesAjustes",s.observacoesAjustes||"");document.getElementById("sessionTitle").textContent=s.meta.os?`Calibração OS ${s.meta.os}`:"Nova calibração";document.getElementById("sessionStatus").textContent=s.id && db.sessions.some(x=>x.id===s.id)?"Salva":"Nova";document.querySelectorAll("[data-select-report]").forEach(x=>x.checked=s.selectedReports.includes(x.dataset.selectReport));renderReportTabs(s);renderAllTables(s)}
...
function resetNewCalibrationForm(){const s=blankSession();currentId=s.id;fillSession(s);const obs=document.getElementById("observacoesAjustes");if(obs)obs.value="";document.getElementById("sessionTitle").textContent="Nova calibração";document.getElementById("sessionStatus").textContent="Nova";window.scrollTo?.(0,0)}
function saveCurrent(){let s=getCurrent();if(!s){s=blankSession();currentId=s.id;db.sessions.push(s)}collectMeta(s);collectTables(s);collectObservation(s);s.status="Salva";s.updatedAt=new Date().toISOString();saveDB();alert("Calibração salva com sucesso.");resetNewCalibrationForm()}
...
function inpClass(v,attrs,cls=""){return `<input class="cell ${cls}" value="${esc(v)}" ${attrs}>`}
...
function renderReportTabs(s){const tabs=document.getElementById("reportTabs");if(!tabs)return;const names={altimetro:"Altímetro",integracao:"Integração",encoder:"Encoder",transponder:"Transponder",airdata:"Air Data"};tabs.innerHTML=s.selectedReports.map((r,i)=>`<div class="tab-wrap"><button class="tab ${r===currentReport||(!currentReport&&i===0)?"active":""}" data-report="${r}">${names[r]}</button><button class="tab-delete" type="button" data-delete-report="${r}" title="Apagar relatório">×</button></div>`).join("");if(!s.selectedReports.includes(currentReport))currentReport=s.selectedReports[0]||"altimetro";document.querySelectorAll("#reportTabs .tab").forEach(b=>b.addEventListener("click",()=>{currentReport=b.dataset.report;document.querySelectorAll("#reportTabs .tab").forEach(x=>x.classList.toggle("active",x===b));document.querySelectorAll(".report").forEach(x=>x.classList.add("hidden"));document.getElementById("report-"+currentReport)?.classList.remove("hidden")}));document.querySelectorAll("#reportTabs .tab-delete").forEach(b=>b.addEventListener("click",e=>{e.stopPropagation();deleteReportToTrash(b.dataset.deleteReport)}));document.querySelectorAll(".report").forEach(x=>x.classList.toggle("hidden",x.id!=="report-"+currentReport));const active=document.getElementById("report-"+currentReport);if(active){let old=active.querySelector(".report-delete-bar");if(old)old.remove();active.insertAdjacentHTML("afterbegin",`<div class="report-delete-bar"><button type="button" class="ghost danger" onclick="deleteReportToTrash('${currentReport}')">Apagar este relatório</button></div>`);}}
...
window.openSession=openSession;window.showView=showView;window.restoreReport=restoreReport;window.deleteReportForever=deleteReportForever;window.deleteReportToTrash=deleteReportToTrash;

function printReport(){
 let s=getCurrent();
 if(!s){alert("Abra ou salve uma calibração antes de imprimir.");return}
 collectMeta(s);collectTables(s);collectObservation(s);saveDB();
 try{
   const blob=AviationPDF.pdfFromSession(s);
   const url=URL.createObjectURL(blob);
   const file=new File([blob],`Laudo-Calibracao-OS-${(s.meta.os||"sem-OS").replace(/[^a-z0-9_-]/gi,"_")}.pdf`,{type:"application/pdf"});
   if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){
      navigator.share({files:[file],title:"Laudo de calibração",text:"Laudo Aviation Center Calibrações"}).catch(()=>{});
      return;
   }
   const a=document.createElement("a");a.href=url;a.target="_blank";a.rel="noopener";a.textContent="Abrir PDF";a.style.display="none";document.body.appendChild(a);a.click();a.remove();
   setTimeout(()=>URL.revokeObjectURL(url),60000);
 }catch(e){console.error(e);alert("Não foi possível gerar o PDF neste momento.");}
}
...
async function downloadReport(){
 let s=getCurrent();
 if(!s){alert("Abra ou salve uma calibração antes de baixar o relatório.");return}
 collectMeta(s);collectTables(s);collectObservation(s);saveDB();
 try{
   const blob=AviationPDF.pdfFromSession(s);
   const filename=`Laudo-Calibracao-OS-${(s.meta.os||"sem-OS").replace(/[^a-z0-9_-]/gi,"_")}.pdf`;
   const file=new File([blob],filename,{type:"application/pdf"});
   if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({files:[file],title:"Laudo de calibração",text:"Salvar ou compartilhar o laudo"});return}
   const url=URL.createObjectURL(blob);
   ...
   setTimeout(()=>URL.revokeObjectURL(url),60000);
 }catch(e){console.error(e);alert("Não foi possível preparar o PDF.");}
}
...
document.getElementById("saveCalibration")?.addEventListener("click",saveCurrent);document.getElementById("saveReport")?.addEventListener("click",saveCurrent);document.getElementById("printReport")?.addEventListener("click",printReport);
