...
let db=loadDB();db.trash=Array.isArray(db.trash)?db.trash:[];purgeExpiredTrash();
let currentId=null;
let currentReport="altimetro";
function migrateSession(s){s.meta??={};s.meta.equipamento??="";s.selectedReports=Array.isArray(s.selectedReports)&&s.selectedReports.length?s.selectedReports:["altimetro","integracao","encoder","transponder","airdata"];s.reports??={};s.reports.airdata??={rows:AIR_DATA_POINTS.map(()=>({valor:""})),leak:""};if(!Array.isArray(s.reports.airdata.rows)||s.reports.airdata.rows.length!==AIR_DATA_POINTS.length)s.reports.airdata.rows=AIR_DATA_POINTS.map((_,i)=>s.reports.airdata.rows?.[i]||({valor:""}));if(!s.reports.transponder||!Array.isArray(s.reports.transponder.suppression)||!Array.isArray(s.reports.transponder.sensitivity)||!Array.isArray(s.reports.transponder.reception)){const d=blankSession().reports.transponder;const old=s.reports.transponder||{};s.reports.transponder={...d,radio:old.radio||d.radio,power:old.power||d.power,other:typeof old.other==="string"?old.other:(Array.isArray(old.other)?old.other.map(r=>Array.isArray(r)?`${r[0]} — ${r[2]||""}`:String(r)).join("\n"):""),modeS:old.modeS||d.modeS};}return s}
...
function blankSession(){return {id:uid(),status:"Nova",createdAt:new Date().toISOString(),selectedReports:["altimetro","integracao","encoder","transponder","airdata"],meta:{os:"",data:new Date().toISOString().slice(0,10),cliente:"",validade:"2 anos",fabricante:"",pn:"",sn:"",prefixo:"",tecnico:"",equipamento:""},reports:{altimetro:{friction:ALT_FRICTION.map(()=>({sem:"",com:""})),scale:ALT_SCALE.map(()=>({lido:""})),leak:{pressao:"14,942",lido:""},baro:BARO.map(x=>({valor:x[0],diferenca:x[1],lido:""})),hyst:[{lidoDec:"",lidoCresc:"",dif:"",altitude:10000},{lidoDec:"",lidoCresc:"",dif:"",altitude:8000}]},integracao:INT_ALT.map((a,i)=>({teste:i+1,altitude:a,altimetro:"",transponder:""})),encoder:ENC_NOM.map((n,i)=>({teste:i+1,altitude:INT_ALT[i],nominal:n,atual:""})),transponder:{radio:[
...
function fillSession(s){currentId=s.id; set("os",s.meta.os);set("data",s.meta.data);set("cliente",s.meta.cliente);set("validade",s.meta.validade);set("fabricante",s.meta.fabricante);set("pn",s.meta.pn);set("sn",s.meta.sn);set("prefixo",s.meta.prefixo);set("tecnico",s.meta.tecnico);set("equipamento",s.meta.equipamento);document.getElementById("sessionTitle").textContent=s.meta.os?`Calibração OS ${s.meta.os}`:"Nova calibração";document.getElementById("sessionStatus").textContent=s.id && db.sessions.some(x=>x.id===s.id)?"Salva":"Nova";document.querySelectorAll("[data-select-report]").forEach(x=>x.checked=s.selectedReports.includes(x.dataset.selectReport));renderReportTabs(s);renderAllTables(s)}
function collectMeta(s){["os","data","cliente","validade","fabricante","pn","sn","prefixo","tecnico","equipamento"].forEach(k=>{const el=document.getElementById(k);if(el)s.meta[k]=el.value.trim()});s.selectedReports=[...document.querySelectorAll("[data-select-report]:checked")].map(x=>x.dataset.selectReport)}
function saveCurrent(){let s=getCurrent();if(!s){s=blankSession();currentId=s.id;db.sessions.push(s)}collectMeta(s);collectTables(s);s.status="Salva";s.updatedAt=new Date().toISOString();saveDB();alert("Calibração salva com sucesso.");newSession()}
...
function renderAllTables(s){renderFriction(s);syncScaleFromFriction(s);renderScale(s);renderLeak(s);renderBaro(s);renderHyst(s);renderIntegration(s);renderEncoder(s);renderTransponder(s);renderAirData(s);renderReportTabs(s)}
...
function renderScale(s){syncScaleFromFriction(s);let h="<table class='data-table'><thead><tr><th>Altitude (pés)</th><th>Pressão equivalente (pol Hg)</th><th>Valor lido</th><th>Tolerância ± pés</th></tr></thead><tbody>";const p=["31,018","29,921","29,385","28,856","28,335","27,821","26,817","25,842","23,978","22,225","20,577","19,029","17,577","16,216","14,942","13,75"];ALT_SCALE.forEach((a,i)=>{const fi=ALT_FRICTION.indexOf(a);const ro=fi>=0?`readonly title="Copiado automaticamente do Valor com vibração do Teste de Atrito (${a} pés)"`:``;h+=`<tr><td>${a}</td><td>${p[i]}</td><td>${inp(s.reports.altimetro.scale[i].lido,`data-r="scale" data-i="${i}" ${ro}`)}</td><td>${SCALE_TOL[i]}</td></tr>`});document.getElementById("scaleTable").innerHTML=wrap(h+"</tbody></table>")}
...
function renderHyst(s){
  ...
  [["1º ponto de teste — 50% altitude máxima (10000 pés)",10000,75],["2º ponto de teste — 40% altitude máxima (8000 pés)",8000,75]].forEach((r,i)=>h+=`<tr><td>${r[0]}</td><td>${inp(s.reports.altimetro.hyst[i].lidoDec,`data-r="hyst" data-i="${i}" data-k="lidoDec" readonly title="Copiado automaticamente do Erro de Escala na mesma altitude (${r[1]} pés)"`)}</td><td>${inp(s.reports.altimetro.hyst[i].lidoCresc,`data-r="hyst" data-i="${i}" data-k="lidoCresc"`)}</td><td>${inp(s.reports.altimetro.hyst[i].dif,`data-r="hyst" data-i="${i}" data-k="dif" readonly title="Calculado: valor lido (pressão crescente) - valor lido durante o teste de erro de escala (pressão decrescente)"`)}</td><td>${r[2]}</td></tr>`);
...
function renderReportTabs(s){const tabs=document.getElementById("reportTabs");if(!tabs)return;const names={altimetro:"Altímetro",integracao:"Integração",encoder:"Encoder",transponder:"Transponder",airdata:"Air Data"};tabs.innerHTML=s.selectedReports.map((r,i)=>`<button class="tab ${r===currentReport||(!currentReport&&i===0)?"active":""}" data-report="${r}">${names[r]}</button>`).join("");if(!s.selectedReports.includes(currentReport))currentReport=s.selectedReports[0]||"altimetro";document.querySelectorAll("#reportTabs .tab").forEach(b=>b.addEventListener("click",()=>{currentReport=b.dataset.report;document.querySelectorAll("#reportTabs .tab").forEach(x=>x.classList.toggle("active",x===b));document.querySelectorAll(".report").forEach(x=>x.classList.add("hidden"));document.getElementById("report-"+currentReport)?.classList.remove("hidden")}));document.querySelectorAll(".report").forEach(x=>x.classList.toggle("hidden",x.id!=="report-"+currentReport))}
...
function renderTrash(){
 ...
 el.innerHTML=db.trash.slice().reverse().map(x=>{
   const s=db.sessions.find(y=>y.id===x.sessionId);
   const days=Math.max(1,Math.ceil((new Date(x.expiresAt).getTime()-now)/86400000));
   return `<div class="session-row"><div><b>${esc(x.label)}</b><small>OS ${esc(s?.meta?.os||"—")} · Prefixo ${esc(s?.meta?.prefixo||"—")}</small><div class="trash-meta">Exclusão definitiva em ${days} dia(s).</div></div><div class="actions"><button class="secondary" onclick="restoreReport('${x.id}')">Restaurar</button><button class="ghost danger" onclick="deleteReportForever('${x.id}')">Excluir definitivamente</button></div></div>`;
...
function showView(v){
 ...
 const titles={dashboard:["Dashboard","Gerenciamento de calibração"],session:["Nova calibração","Preenchimento dos laudos"],sessions:["Sessões","Calibrações salvas"],instruments:["Instrumentos","Instrumentos registrados nas sessões"],clients:["Clientes","Clientes e histórico das aeronaves"],aircraft:["Aeronaves","Histórico de serviços por prefixo"],equipment:["Equipamentos","Equipamentos padrão utilizados"],history:["Histórico","Histórico completo de serviços"],trash:["Lixeira","Relatórios apagados nos últimos 7 dias"],backup:["Backup","Dados locais do Aviation Center Calibrações"],settings:["Configurações","Configurações do sistema"]};
 ...
 if(v==="instruments")renderInstruments();
...
function validityStatus(s){const d=new Date(s.meta?.data||"");if(!Number.isFinite(d.getTime()))return {label:"Sem data",cls:""};d.setFullYear(d.getFullYear()+2);const now=new Date();const diff=Math.ceil((d-now)/86400000);if(diff<0)return {label:"Vencido",cls:"status-expired"};if(diff<=30)return {label:`Vence em ${diff} dia(s)`,cls:"status-warning"};if(diff<=90)return {label:`Vence em ${diff} dia(s)`,cls:"status-warning"};return {label:"Vigente",cls:"status-done"}}
...
function renderAircraft(){const el=document.getElementById("aircraftList");if(!el)return;const map={};db.sessions.forEach(s=>{const p=(s.meta?.prefixo||"").trim()||"Sem prefixo";(map[p]??=[]).push(s)});const arr=Object.entries(map).sort((a,b)=>a[0].localeCompare(b[0]));el.innerHTML=arr.length?arr.map(([prefix,ss])=>{const latest=ss.slice().sort((a,b)=>String(b.meta?.data||"").localeCompare(String(a.meta?.data||"")))[0];const st=validityStatus(latest);return `<div class="session-row"><div><b>${esc(prefix)}</b><small>Cliente: ${esc(latest.meta?.cliente||"Sem cliente")} · Último serviço: ${fmtDate(latest.meta?.data)} · ${ss.length} registro(s)</small><div class="trash-meta ${st.cls}">${st.label}</div></div><button class="secondary" onclick="openSession('${latest.id}')">Abrir ficha</button></div>`}).join(""):`<div class="empty">Nenhuma aeronave registrada ainda.</div>`}
function renderInstruments(){const el=document.getElementById("instrumentsList");if(!el)return;const map={};db.sessions.forEach(s=>{const key=`${s.meta?.pn||"Sem P/N"}|${s.meta?.sn||"Sem S/N"}`;(map[key]??=[]).push(s)});const arr=Object.entries(map);el.innerHTML=arr.length?arr.map(([key,ss])=>{const [pn,sn]=key.split("|");const latest=ss.slice().sort((a,b)=>String(b.meta?.data||"").localeCompare(String(a.meta?.data||"")))[0];return `<div class="session-row"><div><b>P/N ${esc(pn)}</b><small>S/N ${esc(sn)} · Prefixo: ${esc(latest.meta?.prefixo||"—")} · Cliente: ${esc(latest.meta?.cliente||"—")}</small></div><button class="secondary" onclick="openSession('${latest.id}')">Abrir</button></div>`}).join(""):`<div class="empty">Nenhum instrumento registrado ainda.</div>`}
function renderEquipment(){const el=document.getElementById("equipmentList");if(!el)return;const map={};db.sessions.forEach(s=>{const e=(s.meta?.equipamento||"").trim();if(e)(map[e]??=[]).push(s)});const arr=Object.entries(map).sort((a,b)=>a[0].localeCompare(b[0]));el.innerHTML=arr.length?arr.map(([name,ss])=>`<div class="session-row"><div><b>${esc(name)}</b><small>${ss.length} sessão(ões) · Último uso: ${fmtDate(ss.slice().sort((a,b)=>String(b.meta?.data||"").localeCompare(String(a.meta?.data||"")))[0].meta?.data)}</small></div></div>`).join(""):`<div class="empty">Nenhum equipamento informado nas sessões.</div>`}
...
function rowHtml(s){return `<div class="session-row"><div><b>OS ${esc(s.meta.os||"—")} — ${esc(s.meta.cliente||"Sem cliente")}</b><small>${esc(s.meta.prefixo||"Sem prefixo")} · Salva · ${s.meta.data||""}</small></div><button class="secondary" onclick="openSession('${s.id}')">Abrir</button></div>`}
...
window.openSession=openSession;window.restoreReport=restoreReport;window.deleteReportForever=deleteReportForever;window.deleteReportToTrash=deleteReportToTrash;

function printReport(){
 let s=getCurrent();
 if(!s){alert("Salve a sessão antes de imprimir.");return}
 collectMeta(s);collectTables(s);saveDB();buildPrint(s);
 // No iPad/PWA, abrir uma nova janela pode ser bloqueado pelo Safari.
 // O sistema prepara o documento na própria página e chama a impressão nativa.
 document.documentElement.classList.add("printing-sgc");
 setTimeout(()=>{try{window.print();}catch(e){alert("Não foi possível abrir a impressão neste dispositivo.");}},120);
}
window.addEventListener("afterprint",()=>document.documentElement.classList.remove("printing-sgc"));
...
document.querySelectorAll(".nav").forEach(b=>b.addEventListener("click",()=>{showView(b.dataset.view);if(b.dataset.view==="trash")renderTrash();}));
window.addEventListener("online",updateNetworkStatus);
window.addEventListener("offline",updateNetworkStatus);
setInterval(updateNetworkStatus,15000);
quickNew.onclick=newSession;newFromList.onclick=newSession;
saveCalibration.onclick=saveCurrent;saveReport.onclick=saveCurrent;printReport.onclick=printReport;
...
document.querySelectorAll("[data-select-report]").forEach(ch=>ch.addEventListener("change",()=>{let s=getCurrent();if(!s)return;collectMeta(s);if(!s.selectedReports.length){ch.checked=true;return}renderReportTabs(s)}));
document.addEventListener("input",e=>{if(e.target.matches("[data-r='friction'][data-k='com']")){let s=getCurrent();if(!s)return;collectMeta(s);collectTables(s);renderScale(s);renderIntegration(s);renderHyst(s)}if(e.target.matches("[data-r='scale']")){let s=getCurrent();if(!s)return;collectMeta(s);collectTables(s);renderIntegration(s);renderHyst(s)}});
