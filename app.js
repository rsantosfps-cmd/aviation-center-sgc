...
function newSession(){currentId=null;const s=blankSession();fillSession(s,false);setReportEditorVisible(false);showView("session");window.scrollTo?.(0,0)}
function fillSession(s,openReports=true){currentId=s.id; set("os",s.meta.os);set("data",s.meta.data);set("cliente",s.meta.cliente);set("validade",s.meta.validade);set("fabricante",s.meta.fabricante);set("pn",s.meta.pn);set("sn",s.meta.sn);set("prefixo",s.meta.prefixo);set("tecnico",s.meta.tecnico);set("observacoesAjustes",s.observacoesAjustes||"");document.getElementById("sessionTitle").textContent=s.meta.os?`Calibração OS ${s.meta.os}`:"Nova calibração";document.getElementById("sessionStatus").textContent=s.id && db.sessions.some(x=>x.id===s.id)?"Salva":"Nova";document.querySelectorAll("[data-select-report]").forEach(x=>x.checked=s.selectedReports.includes(x.dataset.selectReport));if(openReports){setReportEditorVisible(true);renderReportTabs(s);renderAllTables(s)}else{document.querySelectorAll("[data-select-report]").forEach(x=>x.checked=s.selectedReports.includes(x.dataset.selectReport));}}
