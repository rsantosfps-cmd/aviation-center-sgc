...
let currentId=null;
window.ACSGetDB=()=>db;
function mergeSessionData(local,remote){
  const newer=String(local?.updatedAt||local?.createdAt||"")>=String(remote?.updatedAt||remote?.createdAt||"");
  ...
  const mergeValue=(a,b)=>{
    ...
    if(a&&b&&typeof a==="object"&&typeof b==="object"&&!Array.isArray(a)&&!Array.isArray(b)){
      const o={};new Set([...Object.keys(a),...Object.keys(b)]).forEach(k=>o[k]=mergeValue(a[k],b[k]));return o;
    }
    if(isBlank(a)&&!isBlank(b))return b;
    if(!isBlank(a)&&isBlank(b))return a;
    return newer?a:b;
  };
  return mergeValue(local,remote);
}
window.ACSBackendApplyState=(state)=>{
  if(!state||!Array.isArray(state.sessions))return;
  const active=currentId?db.sessions.find(x=>x.id===currentId):null;
  db=state;
  db.trash=Array.isArray(db.trash)?db.trash:[];
  if(active){
    const remote=db.sessions.find(x=>x.id===active.id);
    if(remote){
      const merged=mergeSessionData(active,remote);
      const i=db.sessions.findIndex(x=>x.id===active.id);
      db.sessions[i]=merged;
    }else{
  ...
  updateDashboard();renderSessions();
  if(currentId){
    const current=db.sessions.find(x=>x.id===currentId);
    if(current)fillSession(current,true);
  }
};
...
function updateDashboard(){const a=db.sessions;const total=document.getElementById("dashTotal");const done=document.getElementById("dashDone");const last=document.getElementById("dashLast");const recentEl=document.getElementById("recentSessions");if(total)total.textContent=a.length;if(done)done.textContent=a.length;if(last)last.textContent=a.length?(a[a.length-1].meta?.os||"Sem OS"):"—";if(recentEl){const recent=a.slice(-5).reverse();recentEl.innerHTML=recent.length?recent.map(rowHtml).join(""):`<div class="empty">Nenhuma calibração cadastrada.</div>`;}}
...
function makeCurrentPDFForSession(s, fromCurrentForm=false, reportFilter=null){
  try{
    if(fromCurrentForm){ collectMetaForSession(s); collectTablesForSession(s); collectObservationForSession(s); }
    const blob=AviationPDF.pdfFromSession(s, reportFilter);
    const filename=`Laudo-Calibracao-OS-${(s.meta.os||"sem-OS").replace(/[^a-z0-9_-]/gi,"_")}.pdf`;
    return {blob,s,filename};
...
function printSessionObject(s, reportFilter=null){
  const result=makeCurrentPDFForSession(s,true,reportFilter);if(!result)return;
  const suffix=reportFilter&&reportFilter.length===1?`-${reportFilter[0]}`:"";
  result.filename=result.filename.replace(/\.pdf$/i,`${suffix}.pdf`);
  if(sharePdf(result.blob,result.filename))return;
  openBlobPdf(result.blob,result.filename);
}
function reportLabel(k){
  return ({altimetro:"Altímetro",integracao:"Integração do Transponder e Altitude Encoder",encoder:"Encoder",transponder:"Transponder",airdata:"Air Data"})[k]||k;
}
let printChooserSession=null;
function closePrintChooser(){
  const el=document.getElementById("printChooser");if(!el)return;
  el.classList.add("hidden");el.setAttribute("aria-hidden","true");printChooserSession=null;
}
function openPrintChooser(s){
  if(!s){alert("Abra uma calibração antes de imprimir.");return;}
  const selected=(s.selectedReports||[]).filter(Boolean);
  if(!selected.length){alert("Esta calibração não possui laudos selecionados para impressão.");return;}
  printChooserSession=s;
  const list=document.getElementById("printChooserList");
  list.innerHTML=selected.map(k=>`<label class="print-choice"><input type="checkbox" data-print-report="${esc(k)}" checked><span>${esc(reportLabel(k))}</span></label>`).join("");
  const el=document.getElementById("printChooser");el.classList.remove("hidden");el.setAttribute("aria-hidden","false");
}
function confirmPrintSelection(){
  if(!printChooserSession)return;
  const chosen=[...document.querySelectorAll("[data-print-report]:checked")].map(x=>x.dataset.printReport);
  if(!chosen.length){alert("Selecione pelo menos um laudo para imprimir.");return;}
  const s=printChooserSession;closePrintChooser();printSessionObject(s,chosen);
}
...
function printSessionById(id){
  ...
  currentId=id;openPrintChooser(s);
}
...
document.getElementById("printChooserPrint")?.addEventListener("click",confirmPrintSelection);
document.getElementById("printChooserAll")?.addEventListener("click",()=>document.querySelectorAll("[data-print-report]").forEach(x=>x.checked=true));
document.querySelectorAll("[data-close-print-chooser]").forEach(el=>el.addEventListener("click",closePrintChooser));
...
/* ACS: visual-only Transponder section grouping.
   This does not alter the existing field names, IDs, values, or persistence. */
(function(){
  const TITLES = [
    "RÁDIO FREQUÊNCIA DE RESPOSTA",
    "SUPRESSÃO — MODO 3/A",
    "SENSIBILIDADE DO RECEPTOR",
    "POTÊNCIA DE PICO DE SAÍDA DE RÁDIO FREQUÊNCIA",
    "OUTRAS MEDIÇÕES",
    "PARÂMETROS — MODE A / MODE C",
    "SOMENTE PARA TRANSPONDER MODE S"
  ];
  ...
  function enhance(root){
    if(!root || root.dataset.acsTransponderEnhanced==="1") return;
    const all=[...root.querySelectorAll("*")];
    const headings=all.filter(el=>{
      const txt=normalize(el.textContent);
      return TITLES.some(x=>txt===x) && el.children.length<4;
    });
    headings.forEach(h=>{
      if(h.closest(".acs-transponder-table-section")) return;
      const title=TITLES.find(x=>normalize(h.textContent)===x);
      if(!title) return;

      const section=document.createElement("section");
      section.className="acs-transponder-table-section";
      const titleEl=document.createElement("div");
      titleEl.className="acs-transponder-section-title";
      titleEl.textContent=title;
      section.appendChild(titleEl);

      // Preserve the existing DOM nodes and their controls. We only group the
      // existing section until the next recognized heading.
      const parent=h.parentElement;
      if(!parent) return;
      const children=[...parent.children];
      const start=children.indexOf(h);
      if(start<0) return;
      ...
      for(let i=start+1;i<children.length;i++){
        const n=children[i];
        const nt=normalize(n.textContent);
        if(TITLES.includes(nt)) break;
        section.appendChild(n);
      }
    });
    root.dataset.acsTransponderEnhanced="1";
  }

  window.ACSEnhanceTransponderTables=enhance;
})();
