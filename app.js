...
function blankSession(){return {id:uid(),status:"Rascunho",createdAt:new Date().toISOString(),selectedReports:["altimetro","integracao","encoder","transponder","airdata"],meta:{os:"",data:new Date().toISOString().slice(0,10),cliente:"",validade:"2 anos",fabricante:"",pn:"",sn:"",prefixo:"",tecnico:""},reports:{altimetro:{friction:ALT_FRICTION.map(()=>({sem:"",com:""})),scale:ALT_SCALE.map(()=>({lido:""})),leak:{pressao:"14,942",lido:""},baro:BARO.map(x=>({valor:x[0],diferenca:x[1],lido:""})),hyst:[{lidoDec:"",lidoCresc:"",dif:"",altitude:10000},{lidoDec:"",lidoCresc:"",dif:"",altitude:8000}]},integracao:INT_ALT.map((a,i)=>({teste:i+1,altitude:a,altimetro:"",transponder:""})),encoder:ENC_NOM.map((n,i)=>({teste:i+1,altitude:INT_ALT[i],nominal:n,atual:""})),transponder:{radio:[
...
function fillSession(s){currentId=s.id; set("os",s.meta.os);set("data",s.meta.data);set("cliente",s.meta.cliente);set("validade",s.meta.validade);set("fabricante",s.meta.fabricante);set("pn",s.meta.pn);set("sn",s.meta.sn);set("prefixo",s.meta.prefixo);set("tecnico",s.meta.tecnico);document.getElementById("sessionTitle").textContent=s.meta.os?`Sessão OS ${s.meta.os}`:"Nova sessão de calibração";document.getElementById("sessionStatus").textContent=s.status;document.querySelectorAll("[data-select-report]").forEach(x=>x.checked=s.selectedReports.includes(x.dataset.selectReport));renderReportTabs(s);renderAllTables(s)}
function collectMeta(s){["os","data","cliente","validade","fabricante","pn","sn","prefixo","tecnico"].forEach(k=>s.meta[k]=document.getElementById(k).value.trim());s.selectedReports=[...document.querySelectorAll("[data-select-report]:checked")].map(x=>x.dataset.selectReport)}
...
function printReport(){
 let s=getCurrent();if(!s){alert("Salve a sessão antes de imprimir.");return}
 collectMeta(s);collectTables(s);saveDB();buildPrint(s);
 const w=window.open("","_blank");
 if(!w){alert("O Safari bloqueou a impressão. Permita pop-ups para o site do SGC.");return}
 const cssText=`@page{size:Letter portrait;margin:0}html,body{margin:0;background:#fff;color:#000;font-family:Arial,Helvetica,sans-serif}.print-page{width:216mm;height:279mm;box-sizing:border-box;overflow:hidden;page-break-after:always;background:#fff;padding:10mm}.print-page:last-child{page-break-after:auto}.print-title{text-align:center;font-size:13px;font-weight:700;margin:0 0 6mm}.print-meta{display:grid;grid-template-columns:16mm 1fr 16mm 1fr 16mm 1fr 16mm 1fr;border:1px solid #000;margin-bottom:4mm}.print-meta div{min-height:7mm;padding:1.4mm;border-right:1px solid #000;border-bottom:1px solid #000;overflow:hidden}.print-meta div:nth-child(8n){border-right:0}.print-meta div:nth-last-child(-n+4){border-bottom:0}.print-table{width:100%;border-collapse:collapse;margin-bottom:3mm;table-layout:fixed;font-size:9px}.print-table th,.print-table td{border:1px solid #000;padding:1.5mm;text-align:center;vertical-align:middle}.print-section{font-weight:700;text-align:center;border:1px solid #000;border-bottom:0;padding:1.5mm;margin-top:3mm}.print-grid2{display:grid;grid-template-columns:1fr 1fr;gap:3mm}.print-note{margin-top:3mm;font-size:8px}.sig{position:absolute;bottom:8mm;left:10mm;right:10mm;display:grid;grid-template-columns:1fr 1fr;gap:20mm}.sig div{text-align:center;border-top:1px solid #000;padding-top:2mm}`;
 w.document.open();w.document.write('<!doctype html><html><head><meta charset="utf-8"><title>Laudos SGC</title><style>'+cssText+'</style></head><body>'+printRoot.innerHTML+'</body></html>');w.document.close();
