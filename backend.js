...
// Supabase Auth + REST API. A sincronização sempre faz MERGE local + nuvem.
(function(){
  const C=window.ACS_BACKEND_CONFIG||{};
  const SESSION_KEY="acs_backend_auth_v2";
  const SYNC_META_KEY="acs_backend_sync_meta_v1";
  ...
  function headers(token){return {"apikey":C.anonKey,"Authorization":"Bearer "+token,"Content-Type":"application/json","Accept":"application/json"};}
  ...
  async function login(email,password,localDb){
    ...
    if(!result?.ok) throw new Error(result?.error?.message || "Login realizado, mas a sincronização falhou.");
    return j;
  }
  ...
  async function readCloud(){
    ...
    const rows=await api("/rest/v1/app_state?select=payload,updated_at&owner_id=eq."+encodeURIComponent(a.user.id));
    return rows?.[0]||null;
  }
  ...
  async function syncNow(localDb,reason="manual"){
    ...
    try{
      ...
      // Sempre envia o MERGE completo. Assim dados criados offline são enviados
      // e dados existentes somente na nuvem também chegam ao aparelho.
      await push(merged);

      // Confirma a gravação lendo novamente a nuvem. Isso evita mostrar
      // “sincronizado” quando o upsert foi rejeitado por RLS/rede.
      const verify=await readCloud();
      if(!verify?.payload)throw new Error("A nuvem não confirmou os dados após o envio.");

      if(window.ACSBackendApplyState)window.ACSBackendApplyState(merged);
      const now=new Date().toISOString();
      setMeta({lastSyncAt:now,cloudUpdatedAt:verify.updated_at||now});
      setStatus("Sincronizado com a nuvem • "+new Date(now).toLocaleString("pt-BR"),true);
      return {ok:true,state:merged,cloud:verify};
    }catch(e){
      setStatus("Sincronização pendente: "+e.message,false);
  ...
  async function push(db){
    ...
    await api("/rest/v1/app_state?on_conflict=owner_id",{
      method:"POST",
      headers:{Prefer:"resolution=merge-duplicates,return=minimal"},
      body:JSON.stringify({owner_id:a.user.id,payload:clone(db),updated_at:new Date().toISOString()})
    });
    return true;
  }
  ...
  function updateUI(){
    ...
    const meta=getMeta();
    if(a?.user?.email){
      st.textContent="Conta conectada: "+a.user.email+(meta.lastSyncAt?" • Última sincronização: "+new Date(meta.lastSyncAt).toLocaleString("pt-BR"):"");
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    document.getElementById("backendLogin")?.addEventListener("click",async()=>{
      ...
      try{await login(e,p,window.ACSGetDB?.());if(m)m.textContent="Login realizado e sincronização concluída.";}
