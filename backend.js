...
(function(){
  ...
  function mergeStates(local,cloud){const out=clone(local||{sessions:[],trash:[]});out.sessions=Array.isArray(out.sessions)?out.sessions:[];out.trash=Array.isArray(out.trash)?out.trash:[];const mergeBy=(a,b,key,stamp)=>{const map=new Map((a||[]).map(x=>[x[key],x]));for(const x of (b||[])){const old=map.get(x[key]);if(!old || String(x[stamp]||x.updatedAt||x.deletedAt||"")>String(old[stamp]||old.updatedAt||old.deletedAt||""))map.set(x[key],x)}return [...map.values()]};out.sessions=mergeBy(out.sessions,cloud?.sessions||[],"id","updatedAt");out.trash=mergeBy(out.trash,cloud?.trash||[],"id","deletedAt");return out;}
  async function syncNow(localDb){if(!configured())return {configured:false};if(syncing)return {busy:true};syncing=true;try{let a=getAuth();if(!a?.access_token)return {authenticated:false};let rows=await api("/rest/v1/app_state?select=payload,updated_at&owner_id=eq."+encodeURIComponent(a.user.id));let cloud=rows?.[0]?.payload||null;const merged=cloud?mergeStates(localDb||{sessions:[],trash:[]},cloud):(localDb||{sessions:[],trash:[]});if(cloud && JSON.stringify(merged)!==JSON.stringify(cloud)){await push(merged);}else if(!cloud){await push(merged);}if(window.ACSBackendApplyState && JSON.stringify(merged)!==JSON.stringify(localDb||{}))window.ACSBackendApplyState(merged);setStatus("Sincronizado com a nuvem.",true);return {ok:true,state:merged};}catch(e){setStatus("Backend: "+e.message,false);return {ok:false,error:e};}finally{syncing=false;}}
  async function push(db){const a=getAuth();if(!a?.access_token||!configured())return false;await api("/rest/v1/app_state?on_conflict=owner_id",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify({owner_id:a.user.id,payload:clone(db),updated_at:new Date().toISOString()})});return true;}
  function queuePush(db){if(!configured()||!getAuth()?.access_token)return;clearTimeout(timer);timer=setTimeout(()=>push(db).then(()=>setStatus("Sincronizado com a nuvem.",true)).catch(e=>setStatus("Alteração local; sincronização pendente.",false)),700);}
