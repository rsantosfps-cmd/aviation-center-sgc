...
(function(){
  ...
  async function upsertState(db){
    ...
    const payload=JSON.stringify({owner_id:a.user.id,payload:clone(db),updated_at:new Date().toISOString()});
    // Primeiro tenta atualizar um registro já existente. Depois cria, se necessário.
    // Isso evita depender do comportamento de upsert do PostgREST em ambientes
    // onde a resolução de conflito pode ser restringida pelas políticas/RLS.
    const updated=await api("/rest/v1/app_state?owner_id=eq."+encodeURIComponent(a.user.id),{
      method:"PATCH",
    ...
    try{
      const inserted=await api("/rest/v1/app_state",{
        method:"POST",
        headers:{Prefer:"return=representation"},
        body:payload
      });
      ...
      // Se outra aba/aparelho criou o registro entre PATCH e POST, tenta PATCH uma vez.
      const retry=await api("/rest/v1/app_state?owner_id=eq."+encodeURIComponent(a.user.id),{
        method:"PATCH",
        headers:{Prefer:"return=representation"},
        body:payload
      });
      if(Array.isArray(retry)&&retry.length>0)return retry[0];
      throw e;
    }
  }
  ...
  async function syncNow(localDb,reason="manual"){
    ...
    try{
      ...
      setMeta({lastSyncAt:now,cloudUpdatedAt:verify.updated_at||saved?.updated_at||now});
  ...
  function updateUI(){
    ...
    const meta=getMeta();
    if(a?.user?.email){
      st.textContent="Conta conectada: "+a.user.email+(meta.lastSyncAt?" • Última sincronização: "+new Date(meta.lastSyncAt).toLocaleString("pt-BR"):" • Sincronização pendente");
